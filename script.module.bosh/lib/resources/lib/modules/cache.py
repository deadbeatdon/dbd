...
def _find_cache_version():
    ...
    try:
        curVersion = control.addon('script.module.bosh').getAddonInfo('version')
...
def _find_cache_versionAlt(): #Added to keep track of plugin.video.bosh version
    ...
    try:
        curVersion = control.addon('plugin.video.bosh').getAddonInfo('version')
        if oldVersion != curVersion: 
            with open(versionFile, 'wb') as fh: fh.write(curVersion)
            return True
        else: return False
